# Venous Oxygen Research Scan — 2026-08-13

**Search window:** Thirty-fifth scan in the series, run **one day** after 2026-08-12. Emphasis on work published or first-surfacing after 08-12, with the standard fallback to the best-available venous-specific work already catalogued. Sources swept today: PubMed/PMC, arXiv (physics.med-ph / eess.IV new-listing sweep), bioRxiv/medRxiv, Optica (Biomed. Opt. Express, Optics Letters, Applied Optics), SPIE / *J. Biomed. Opt.*, *Photoacoustics* (ScienceDirect), Springer, Wiley (*Advanced Science*, *J. Biophotonics*), De Gruyter, MDPI (*Sensors/Biosensors*), IEEE Xplore, Nature / *Sci. Rep.* / *Commun. Med.*, ClinicalTrials.gov, plus Semantic Scholar / Google-Scholar-style index snippets and ResearchGate.

**Verification caveat:** Publisher **full-text** pages and several databases were again unreachable for direct `WebFetch` today: **pmc.ncbi.nlm.nih.gov, www.spiedigitallibrary.org, and utsouthwestern.elsevierpure.com returned `EGRESS_BLOCKED`** from the network egress proxy, and the usual publisher hosts (link.springer.com, opg.optica.org, sciencedirect.com, wiley.com) return **HTTP 403** to direct fetch; arxiv.org remains egress-blocked in this environment. The agent proxy itself reported healthy — the blocks are source-/policy-side, not relay failures. Every reference below was therefore cross-checked across **≥2 independent search-index snippets**; per-item notes state what was confirmed and what could not be read directly on a publisher page.

---

## ⚠️ Read this first — day summary

**One item genuinely new to the corpus today, and it is Topic-2 retinal (per-vessel venular sO₂), not systemic SvO₂.** Today's one-day sweep surfaced **one new primary 2026 measurement paper**:

- **Development and validation of a high-resolution hyperspectral imaging system for the retina** — Tran M.H., Pruitt K., Bryarly M., Emordi I., Ali A., Ma L., Fei B. — *Journal of Biomedical Optics* **31**(3):036006 (2026 Mar 1); DOI 10.1117/1.JBO.31.3.036006; PMC12997856. **NEW to the corpus.** A retinal hyperspectral fundus system that extracts **per-vessel retinal artery *and vein* sO₂** simultaneously with vessel diameter, reporting **average venule sO₂ ≈ 58 %** vs arteriole ≈ 98 %, with a deep-learning pansharpening step (RMSE 2.15 ± 0.64, CC 0.96 ± 0.05). This is a *true intravascular venular* optical measurement — added as a ranked Topic-2 entry (§2.5) with an honest "retinal per-vessel, not systemic SvO₂" flag.

**No new venous-specific systemic (SvO₂/ScvO₂) measurement paper today — the fourteenth consecutive scan cycle with none.** All standing Topic-1 anchors and the deep-vein PA anchors are unchanged and carried forward with scores held.
...
*One new item today (§2.5, hyperspectral retinal system).* Other measurement anchors carried forward with scores held.
...
### 2.5 — Development and Validation of a High-Resolution Hyperspectral Imaging System for the Retina *(**NEW this scan** — first surfaced 08-13; per-vessel retinal artery + vein sO₂)*
...
- **Relevance:** 7.5/10 — a new primary 2026 hyperspectral fundus system that measures **per-vessel retinal venule sO₂ (≈58 %)** alongside arteriole sO₂ (≈98 %) and vessel diameter; a true intravascular venular optical readout, though retinal per-vessel, **not systemic SvO₂**.
...
Today produced **one item genuinely new to the corpus** — the Tran et al. hyperspectral retinal imaging system (*J. Biomed. Opt.* 31(3):036006, 2026, §2.5) — and it lands squarely in the **retinal per-vessel venular** sub-cluster rather than on the systemic deep-vein front. It measures true intravascular retinal **vein** sO₂ (≈58 %, matching the accepted venous norm) with a camera-based hyperspectral fundus system and a deep-learning spatial–spectral fusion step, offering a cheaper hardware route to the same per-vessel venous readout that vis-OCT (§2.4) and PA (§2.1/§2.2) reach with more elaborate instrumentation. **No new systemic SvO₂/ScvO₂ measurement paper appeared today — the fourteenth consecutive dry cycle** for true central/peripheral venous work once arterial-only and tissue-level (StO₂) items are excluded.

The two topics remain coupled by one problem — **isolating the venous signal and correcting the confound that corrupts its saturation estimate** — addressed by three complementary strategies, all of which today's new paper touches on the retinal end:
