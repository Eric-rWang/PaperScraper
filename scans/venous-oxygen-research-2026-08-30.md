# Daily Venous-Oxygen Research Scan — 2026-08-30

## ⚠️ Read this first — day summary

- **Scan cadence:** 43rd scan in the series (immediately prior run 2026-08-29). Emphasis on work published or first-surfacing after 08-29.
- **Verification caveat (today):** `WebSearch` was available and productive, but `WebFetch` to scholarly hosts remains **egress-blocked by the environment's network policy** — a direct `WebFetch` of arXiv (arxiv.org/abs/2303.10775) returned `EGRESS_BLOCKED`, identical to the 08-29 run. `$HTTPS_PROXY/__agentproxy/status` shows an allowlist with no scholarly hosts and **no recent relay failures** — the block is policy-side, not a TLS/relay fault. Every item below was therefore **cross-checked across ≥2 independent search-index snippets** rather than read on the publisher page. Items whose full metadata could not be page-verified are flagged **⚠ search-derived**.
- **No new venous-specific optical measurement paper first-surfaced today.** Targeted sweeps across all in-scope modalities — SvO₂/ScvO₂ optical, jugular PA/NIRS, deep-vein/forearm PA, vis-OCT venular oximetry, hyperspectral fundus, DOS/venous-occlusion NIRS, wearable venous PPG, ScvO₂ optoacoustic, August-2026 arXiv PACT — returned **only papers already catalogued** in prior dated files. Each promising hit (APM+ *Adv. Sci.* radial-vein PA; PMC11161372 human-vein PACT; PMC12997856 Tran/Fei hyperspectral retinal; PMC12904531 wearable FD-NIRS ASIC; arXiv:2512.15394 Hybrid-Net; arXiv:2601.01667 ingestible PA light source) was checked against the corpus and confirmed **previously surfaced** (or, for the ingestible light source, out-of-scope enabling hardware rather than venous oximetry). This is a genuine low-publication-volume field, exactly as the last several scans reported — **not** a search gap. I am **not** padding with arterial-only SpO₂ work.
...
- **Novelty:** Pulse-oximeter-analogue sensor over the external/internal jugular; exploits arterio-venous compliance differences to render venous blood detectably pulsatile, estimating SvO₂ in the healthy 60–90 % band across a small (n≈3) subject cohort (reported EJV values 71.1 %, 72.2 %, 70.4 %). Correctly identifies the EJV waveform.
...
- **Year / Venue:** 2024 · *Optics Letters* **49**(10):2669 (opg.optica.org/ol) — venue re-confirmed via Optica index snippet.
...
- **Link / DOI:** https://doi.org/10.1186/s12938-017-0351-x · PMC5437414. ⚠ PMC full text egress-blocked; abstract re-confirmed via index snippet.
- **Relevance:** 8/10 — foundational peripheral venous method.
- **Novelty:** Induces artificial respiration-like modulation of the peripheral vasculature (exploiting arterio-venous compliance contrast) to make venous blood pulsatile, then extracts a venous PPG for regional venous sO₂ (SpvO₂) — the conceptual root of the modulation-based venous-oximetry line.
- **Integration insights:** Method ancestor of the jugular device (§1.1) and the e-tattoo (§1.4); establishes arterio-venous compliance contrast as an exploitable, reproducible signal source.
...
- **Authors / Venue:** Lu group (UT Austin) · 2023 IEEE conference (EMBC-style), IEEE Xplore doc 10340010; PubMed 38083768; MRS Fall 2024 talk EL05.07.03. Re-confirmed via IEEE + MRS snippets.
...
- **Link:** https://pubmed.ncbi.nlm.nih.gov/37827917/ ⚠ egress-blocked; metadata via index snippet.
- **Relevance:** 7/10 — non-invasive ScvO₂ surrogate via transcutaneous jugular NIRS; commercially fielded.
- **Novelty:** Clinical validation of the Mespere VO100 measuring SjvO₂ over the jugular as a non-invasive stand-in for catheter ScvO₂ in critically ill patients. (Supporting pediatric-cardiac data: NIRSijv vs ScvO₂ r = 0.91, bias 2.92 %, LoA −5.2 % to +11 %.)
...
> **Device/regulatory anchor (not ranked):** *Noninvasix LIVOx optoacoustic central venous oxygenation monitor* (ScvO₂ for septic shock; FDA Breakthrough Device Designation) — previously surfaced scan 08-11. Optoacoustic ScvO₂ device milestone; watch for peer-reviewed validation data.
> **Clinical-trial anchors (not ranked, non-optical):** *Capnodynamic non-invasive mixed-venous SvO₂* (NCT06632197) — the most active *venous* clinical effort; watch for first results. *Continuous jugular venous oxygen saturation after cardiac arrest* (NCT06511999, Univ. Pittsburgh; est. completion 2027) — jugular venous validation cohort, non-optical reference.
...
- **Year / Venue:** 2023 · preprint arXiv:2303.10775 (+ SPIE lineage). ⚠ arXiv egress-blocked today (direct `WebFetch` returned `EGRESS_BLOCKED`); abstract re-confirmed on two index snippets.
- **Link:** https://arxiv.org/abs/2303.10775
- **Relevance:** 9/10 — human, in vivo, PA, *venous* (IJV).
- **Novelty:** Dual-wavelength PAT + ultrasound images the internal jugular vein in **7 healthy volunteers**; reported average IJV sO₂ ≈ **72 ± 7 %**, with fast US co-registration enabling temporal sO₂ tracking under physiologic challenges — true venous oximetry at depth, non-invasively.
...
- **Novelty:** Snapshot hyperspectral fundus imager (hyperspectral camera + high-res RGB camera + beamsplitter + imaging endoscope) recovering vessel diameter *and* oxygenation simultaneously; a **deep-learning pansharpening** step fuses high-spatial and high-spectral channels (RMSE 2.15 ± 0.64, CC 0.96 ± 0.05). Reported venule sO₂ ≈ 58 % matches the accepted retinal venous norm (≈ 57.9 ± 9.9 %), giving physiological face-validity.
...
- **Davenet, Battaglia, Lager, Dargent, Lussey-Lepoutre, Tavitian, Couture, Bridal, Gateau — Simultaneous 3D Co-Registered Perfusion and Oxygenation with ULM, Photoacoustic Imaging, and a Planar Matrix Array** (arXiv:2608.19823, Aug 2026). *First surfaced 08-25; abstract re-confirmed from index snippets:* multispectral PA sO₂ across the physiological **≈60–95 % range at 5 % accuracy using only five wavelengths**, co-registered with ULM sub-diffraction microvascular perfusion on a **single planar matrix array**; validated in vessel-mimicking phantoms and in vivo in mice. *Tissue/vessel-level, small-animal — not human SvO₂.* Its promise for venous work is **hemodynamic vessel-type discrimination** (labeling a vessel venous vs arterial from ULM flow, not sO₂ alone) — the recurring gap in PA venous oximetry. Watch for a human/large-vessel follow-up and a peer-reviewed version.
- **Davenet, Billon, Battaglia, Bridal, Gateau — Sulfate Phantoms to Mimic NIR Photoacoustic Response of Whole Blood at Selected Oxygen Saturations** (*Applied Optics* **65**(6):1974–1982, 2026; arXiv:2512.01458). *Standing metrology lead (previously surfaced).* Recipe for inorganic (sulfate) solutions matching whole-blood PA absorption **and** Grüneisen coefficient over 700–850 nm at chosen sO₂ — an injectable ground-truth for validating multi-wavelength PA venous oximetry across the venous range. Same Paris/LIB group as the ULM+PA platform above; the two together form a build-and-validate stack for deep-vein PA sO₂.
- **Hybrid-Net — Deep-Learning-Driven Quantitative Spectroscopic PA Imaging for Segmentation and Oxygen Saturation Estimation** (arXiv:2512.15394, Dec 2025). *Previously surfaced.* Joint vessel segmentation + sO₂ without explicit fluence estimation; general PA-oximetry method, not venous-specific. Standing technique lead.
- **3D-PAULM: Integrated Photoacoustic Tomography and Ultrasound Localization Microscopy for Multiparametric Brain and Tumor Imaging** (bioRxiv, 30 Apr 2026). Sibling of the ULM+PA platform, combining PAT + ULM for oxygenation + microvascular flow; small-animal brain/tumor. Reinforces the "oxygenation + flow co-registration" theme. ⚠ search-derived.
- **Wearable FD-NIRS ASIC system** (Feb 2026, PMC12904531) — real-time multi-frequency (50–350 MHz) wearable frequency-domain NIRS on a custom ASIC. *Tissue StO₂, not venous SvO₂.* Previously surfaced; logged for the wearable-hardware trajectory a venous method (§1.4) could ride.
- **An Ingestible Light Source for Deep Photoacoustic Imaging** (Garrett & Wang, arXiv:2601.01667, Jan 2026). *New to search this run but out-of-scope as a venous measurement* — an enabling deep-PA illumination approach (imaging through a ~12 cm phantom); no oximetry, let alone venous. Logged only as a deep-tissue-access enabling-hardware watch item, **not** ranked.

---

## Cross-topic synthesis

1. **Another dry cycle for true-venous work — and that is the honest headline.** No new human/systemic non-invasive venous (SvO₂/ScvO₂) measurement paper first-surfaced today, continuing the pattern the 08-23, 08-25 and 08-29 scans reported. The best Topic-1 work (jugular optical sensing §1.1, IJV DNN/Monte-Carlo §1.2, peripheral pulse-modulation §1.3, e-tattoo §1.4, jugular NIRS §1.5) is unchanged since June. This is a real publication-volume gap, not a search artifact.

2. **Where 2026 momentum actually sits: oxygenation + flow co-registration.** The Davenet/Gateau ULM+PA platform (arXiv:2608.19823) and its bioRxiv 3D-PAULM sibling mark a clear trend — pairing **photoacoustic sO₂** with **ULM/Doppler perfusion** on a shared array. For *venous* monitoring the hardest unsolved problem is not measuring sO₂ but **knowing which vessel is the vein**; hemodynamic co-registration offers a physics-based discriminator that pure spectral methods lack. This group is building the full stack — imaging platform (2608.19823) plus a matched validation phantom (sulfate paper, *Appl. Opt.* 65(6):1974) — the most coherent single-lab program adjacent to deep-vein PA oximetry right now.

3. **Two mature accuracy anchors still frame the field.** APM+ (§2.2, deep-tissue, artery-referenced fluence correction, 2.9 % error) and ADS-vis-OCT (§2.4, per-vessel retinal venous sO₂, ~2.3 % repeatability) bracket the state of the art from the deep-tissue and superficial-bed ends. Both hinge on the same core fix — controlling **spectral coloring / unknown fluence** — which is exactly what the ULM+PA co-registration and §1.2's Monte-Carlo priors also target. The 08-13 hyperspectral fundus system (§2.5) adds a cheaper camera-based route to the same per-vessel venular readout.

4. **The near-term opportunity is porting, not discovering.** With Topic-1 clinical targets (IJV, radial/peripheral veins) stable and Topic-2 techniques advancing, the highest-leverage move is transferring Topic-2 advances — fluence correction, adaptive spectral decoloring, oxygenation–flow co-registration, sulfate-phantom metrology — onto the Topic-1 venous targets. The one genuinely-new preprint surfaced this run (ingestible PA light source, arXiv:2601.01667) fits this framing: it extends *depth access* for PA but adds no oximetry, and is only useful to the venous program if paired with a fluence-corrected sO₂ method like APM+.

5. **Confidence caveat.** `WebFetch` to all scholarly hosts was egress-blocked today (a direct arXiv fetch returned `EGRESS_BLOCKED`), so all metadata is search-snippet-derived (each item cross-checked on ≥2 snippets). Publisher-page verification should be repeated on the next scan where direct fetching is permitted.

---
*Scan generated 2026-08-30. Sources located and cross-verified via web search (≥2 independent index snippets per item); direct publisher/preprint-page fetching unavailable this run due to the environment's network egress policy (arXiv confirmed `EGRESS_BLOCKED` via direct WebFetch; PMC/PubMed, Optica, Wiley, De Gruyter, SPIE historically blocked). Metadata flagged ⚠ where it could not be confirmed on the source page.*
