# Venous Oxygen Research Scan — 2026-07-21

**Search window:** Twenty-sixth scan in the series, run **four days** after 2026-07-17 (no scans on 07-18, 07-19, 07-20). Emphasis on work published or first-surfacing after 07-17, with the standard fallback to the best-available venous-specific work already catalogued. Sources searched today: PubMed/PMC, arXiv (incl. physics.med-ph new-listing sweep), bioRxiv/medRxiv, Optica (Biomed. Opt. Express, Optics Letters, Applied Optics), SPIE / *J. Biomed. Opt.*, *Photoacoustics* (ScienceDirect), IEEE Xplore, Nature / *Sci. Rep.* / *Commun. Med.*, Wiley (*Advanced Science*, *J. Biophotonics*), Springer, De Gruyter, MDPI, ClinicalTrials.gov, ResearchGate, Semantic Scholar, ADS, and lab publication pages (e.g. fei-lab.org / Quantitative Bioimaging Laboratory).

**Verification caveat:** As in every prior scan, publisher **full-text** pages — including arxiv.org (403 on the physics.med-ph 2507 listing today), pmc.ncbi.nlm.nih.gov, opg.optica.org, advanced.onlinelibrary.wiley.com, the Nature domains, and fei-lab.org — returned **HTTP 403** to direct `WebFetch` in this environment today. Every reference below was therefore cross-checked across **at least two independent search-index snippets** (Google Scholar-style result blocks + ADS/ResearchGate/PMC/SPIE listings); per-item notes state what was confirmed and what could not be read directly on a publisher page.

---

## ⚠️ Read this first — day summary

**No new venous-specific *primary* paper today — the fifth consecutive scan cycle with none.** Consistent with the series' standing observation, truly novel non-invasive **venous**-optical work publishes at very low volume, and today's four-day sweep produced no new SvO₂/ScvO₂ demonstration. The venous top-5 sets for both topics are unchanged from 07-17 and carried forward with scores held.

The only genuinely **new-to-corpus** items this cycle are two **SPIE 2026 hyperspectral whole-blood** papers from the Quantitative Bioimaging Laboratory (fei-lab.org) — an *algorithm-comparison* study for oxygen-saturation estimation in **whole blood** by diffuse-reflectance hyperspectral imaging (≈March 2026), and a **high-speed reflectance-based** hyperspectral sO₂ approach (≈April 2026). Both are **in-vitro / tissue-level metrology**, **not** venous measurements, and are logged only as **standing context** (§ Standing context leads) with an honest metadata flag — publisher pages were 403-blocked and full author lists could not be read directly. They matter to the venous program the way the sulfate-phantom paper does: as blood-optics ground-truth/algorithm work that underpins, but does not itself perform, venous oximetry.

Everything else returned today is **previously surfaced** or out-of-scope: the IJV-PA anchor (arXiv:2303.10775), APM+ radial-vein PA oximetry (Sastry et al., *Adv. Sci.* 2026), the Alqahtani jugular optical sensor (De Gruyter CDBME), the IJV DNN + Monte-Carlo work (Optics Letters), Mespere VO100 jugular NIRS, the McDiarmid MRI iSvO₂ model, the SSL intra-cardiac MR oximetry preprint, the retinal vis-OCT / hyperspectral line, PACT venous-flow, Hybrid-Net (arXiv:2512.15394), the APRECOT MSOT-prior preprint (arXiv:2606.16835), the DPF-model preprint (arXiv:2602.00283), and the sulfate blood-phantom paper (arXiv:2512.01458 / *Appl. Opt.*). No new primary venous demonstration.
...
*No new venous-specific primary today.* The standing set carried forward with scores held; two new but non-venous hyperspectral items are logged under Standing context leads.
...
- **★ NEW this cycle — "Algorithm comparison for oxygen saturation estimation in whole blood using diffuse reflectance hyperspectral imaging"** (Quantitative Bioimaging Laboratory / fei-lab.org) — *Proc. SPIE*, ≈March 2026. **In-vitro whole-blood metrology**, benchmarking sO₂-estimation algorithms on diffuse-reflectance hyperspectral spectra; **not a venous measurement**. Relevant as blood-optics algorithm/ground-truth work analogous to the sulfate-phantom line (§2.5). *Title/venue/approx-date cross-confirmed via two search snippets + lab publications page; SPIE full text and full author list 403-blocked and **unverified** (flagged).*
- **★ NEW this cycle — "Reflectance-based approach for oxygen saturation estimation using high-speed hyperspectral imaging"** (Quantitative Bioimaging Laboratory) — *Proc. SPIE*, ≈April 2026. **High-speed reflectance hyperspectral sO₂**, tissue/whole-blood level; **not venous-specific**. Of interest for the wearable/real-time imaging form factor (cf. §1.1). *Metadata cross-confirmed via two search snippets; SPIE full text and author list 403-blocked and **unverified** (flagged).*
- **Machine-learning-enabled multiple-illumination quantitative optoacoustic oximetry imaging in humans** (PMC9203099) — learned spectral decoloring for real-time quantitative sO₂; general optoacoustic oximetry, not venous-specific. Previously noted; carried as a technique lead.
...
1. **Sparseness confirmed, not padded — fifth consecutive cycle with no new venous primary.** A four-day sweep across PubMed, arXiv (incl. the physics.med-ph new-listing page), bioRxiv/medRxiv, Optica, SPIE, Wiley, Nature and MDPI produced **no new non-invasive venous-O₂ measurement paper** post-dating 07-17. The Topic-1 (true SvO₂) and Topic-2 (optical venous) top-5 sets are unchanged and carried forward with scores held. The §2.1 IJV-PA anchor, the §2.2 APM+ radial-vein result, and the §2.3 retinal vis-OCT result remain the anchors; nothing newer displaces them.

2. **Today's only new signal is blood-optics metrology, not measurement.** The two SPIE 2026 hyperspectral items are **whole-blood / tissue-level** algorithm and hardware work — the same *category* as the sulfate-phantom paper (§2.5): they sharpen the tools (algorithm benchmarking, high-speed reflectance) that a venous method would eventually run on, but neither measures a vein. Scored out of the top-5 and flagged as metadata-unverified (SPIE 403). They reinforce, rather than change, the map.

3. **The central obstacle is unchanged: absolute quantification under unknown, wavelength- and depth-dependent fluence.** Every live thread converges on it from a different side — internal-reference calibration (APM+ §2.2; non-optically the MRI RV/LV T₂ ratio §1.4), global anatomical priors (APRECOT), better forward physics (DPF model), fluence-free deep learning (Hybrid-Net; §1.2 DNN+Monte-Carlo), and blood-mimicking phantoms/algorithm benchmarking across the venous range (§2.5 + today's hyperspectral items). A clinically accurate deep-vein SvO₂ system will likely **stack** at least two of these; no single paper yet does.

4. **MRI is emerging as the referee for the label problem.** T₂/intra-cardiac CMR oximetry (§1.3 SSL, §1.4 iSvO₂) measures true mixed-venous SvO₂ non-invasively and reference-grade — the natural validation target the jugular optical sensor (§1.1) and deep-vein PA/APM+ (§2.1–2.2) chronically lack. The SSL + uncertainty recipe (§1.3) is portable to the label-starved optical side.
...
*Scan generated 2026-07-21 (UTC). No new venous-specific primary paper; both topic top-5 sets carried forward with scores unchanged. Two new-to-corpus SPIE 2026 hyperspectral whole-blood items logged as standing context with metadata flagged unverified (SPIE/publisher pages 403). Publisher full-text access was 403-blocked throughout this environment; every reference was cross-checked across ≥2 independent search-index snippets, with per-item verification notes and flags above.*
