# Daily Venous-Oxygen Research Scan — 2026-08-31

## ⚠️ Read this first — day summary

- **Scan cadence:** 44th scan in the series (immediately prior run 2026-08-30). Emphasis on work published or first-surfacing after 08-30.
- **Verification caveat (today):** `WebSearch` was available and productive, but `WebFetch` to scholarly hosts remains **egress-blocked by the environment's network policy** — a direct `WebFetch` of arXiv (arxiv.org/abs/2512.15394) returned `EGRESS_BLOCKED`, identical to the 08-29/08-30 runs. Every item below was therefore **cross-checked across ≥2 independent search-index snippets** rather than read on the publisher page. Items whose full metadata could not be page-verified are flagged **⚠ search-derived**.
...
- **One genuinely new-to-corpus item this run** (a *technique-transfer* lead, not true venous): **Apple's PPGen / Hybrid Amortized Inference** (arXiv:2510.02073, now also *IOP* / 3049-477X/ae4a96) — a physics-grounded PPG forward model + fast inverse that estimates interpretable optical/physiological tissue parameters, including a blood-volume-dynamics term separable into vascular compartments. Logged as a standing lead below with an honest relevance score; it is arterial/general PPG today, **not** a venous measurement.
...
- **★ NEW to corpus — Apple ML: PPGen + Hybrid Amortized Inference — *Inferring Optical Tissue Properties from Photoplethysmography using Hybrid Amortized Inference*** (arXiv:2510.02073, Oct 2025; peer-reviewed at *IOP* 10.1088/3049-477X/ae4a96). *Technique-transfer lead — arterial/general PPG today, NOT a venous measurement (relevance to true venous ≈ 4/10, but architecturally important).* **PPGen** is a physically grounded PPG forward model built on light-transport simulation that maps waveform shape to interpretable biophysical parameters — **9 static** (e.g. melanin, arterial oxygenation, vessel/tissue optics) plus **2 dynamic blood-volume terms** over a pulse; **HAI** is a fast, misspecification-robust amortized inverse. **Why it is in-scope despite being arterial-labeled:** the compartment-resolved blood-volume model is the physics-grounded modern successor to the arterio-venous-compliance modulation idea underlying §1.1/§1.3/§1.4 — extending PPGen's dynamic term to an explicit *venous* compartment (or fitting it under venous-modulation stimulation) is a concrete, well-posed route to model-based SpvO₂. Validated in silico only so far. ⚠ metadata search-derived (arXiv + IOP + Apple ML snippets).
...
1. **Another dry cycle for true-venous work — and that is the honest headline.** No new human/systemic non-invasive venous (SvO₂/ScvO₂) measurement paper first-surfaced today, continuing the pattern the 08-23, 08-25, 08-29 and 08-30 scans reported. The best Topic-1 work (jugular optical sensing §1.1, IJV DNN/Monte-Carlo §1.2, peripheral pulse-modulation §1.3, e-tattoo §1.4, jugular NIRS §1.5) is unchanged since June. This is a real publication-volume gap, not a search artifact.

2. **The one new-to-corpus item this run is a modeling lead, not a measurement.** Apple's PPGen/HAI (standing list) matters not because it does venous oximetry — it does not — but because it revives, in a rigorous physics-plus-ML form, the arterio-venous *compartment-separation* idea that Topic-1's whole modulation lineage (§1.1/§1.3/§1.4) rests on. The near-term test is whether its dynamic blood-volume term can be conditioned on a venous compartment or venous-modulation stimulus to yield model-based SpvO₂. It is honestly scored low (~4/10) for *venous relevance today* but is the most interesting adjacent methods development since the ULM+PA platform.
...
5. **The near-term opportunity is porting, not discovering.** With Topic-1 clinical targets (IJV, radial/peripheral veins) stable and Topic-2 techniques advancing, the highest-leverage move is transferring Topic-2 advances — fluence correction, adaptive spectral decoloring, oxygenation–flow co-registration, sulfate-phantom metrology — and now PPGen-style compartment-resolved forward modeling onto the Topic-1 venous targets.
