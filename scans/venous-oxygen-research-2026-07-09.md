# Venous Oxygen Research Scan — 2026-07-09
...
**Prior scans in this repository:** 2026-06-10, 06-11, 06-14, 06-16, 06-17, 06-18, 06-19, 06-21, 06-22, 06-24, 06-25, 06-29, 06-30, 07-01, 07-03, 07-04, 07-05, 07-07, 07-08. Every paper entered in those files — plus all items in their "previously surfaced," "honorable mentions," "reviewed and excluded" and "standing leads" sections — has already been catalogued and is not re-entered here except where the task's "top 5 per topic" mandate requires re-listing the best-available landmark work, in which case it is **explicitly flagged as previously surfaced**.
**Search window:** This is the **twentieth scan** in the series, run **one day** after 2026-07-08. Emphasis on work published or first-surfacing after 07-08, with the standard fallback to the best-available venous-specific work already catalogued. Sources searched today: PubMed/PMC, arXiv, bioRxiv/medRxiv, Optica (Biomed. Opt. Express, Optics Letters, Applied Optics), SPIE / *J. Biomed. Opt.*, *Photoacoustics* (ScienceDirect), IEEE Xplore, Nature / *Sci. Rep.*, Wiley (*Advanced Science*), Elsevier, IOPscience, Springer, De Gruyter, Frontiers, MDPI, ClinicalTrials.gov, ResearchGate, Semantic Scholar, ADS, Europe PMC.
**Verification caveat:** As in every prior scan, publisher **full-text** pages — including arxiv.org, opg.optica.org, pmc.ncbi.nlm.nih.gov, sciencedirect.com and the Wiley/Nature domains — returned **HTTP 403** to direct `WebFetch` in this environment today (re-confirmed on PMC12869027 today). Every reference below was therefore cross-checked across **at least two independent search-index sources**; per-item notes state what was confirmed and what could not be read directly on a publisher page.

---

## ⚠️ Read this first — day summary (ONE new technique paper; NO new venous-specific primary)

This is a **one-day short-cycle scan**. Today's exhaustive multi-angle search surfaced **no not-previously-catalogued primary paper that measures venous saturation**, but did surface **one genuinely new 2026 methods paper** relevant to the program: an **integrated spectral + depth fluence-compensation** method for quantitative photoacoustic sO₂/THb (Wang et al., *J. Biomed. Opt.* Feb 2026 — WashU / Quing Zhu group). It is demonstrated on **ovarian lesions, not veins**, so it is **not** entered as a ranked venous slot; it is catalogued below as a **new standing technique lead** because depth-dependent fluence correction is the exact obstacle a deep-vein PA-SvO₂ measurement must solve. See §New-today and §2.5-adjacent note.

Repeated searches across every other angle the series tracks — deep-vein/IJV photoacoustic oximetry (arXiv 2303.10775), NIRS/jugular venous oximetry, diffuse-optical/DCS cerebral SvO₂, hyperspectral retinal/tissue oximetry, visible-light OCT, SFDI, wearable optical sensors, capnodynamic mixed-venous methods, and the CMR-T₂ MRI reference — **re-surfaced only papers already catalogued** across the nineteen prior files.

Accordingly, the ranked slots below are the **best-available already-catalogued corpus**, re-listed to satisfy the task's "top 5 per topic" mandate and **each flagged as previously surfaced** with honest relevance scores. No arterial-only SpO₂ padding was added to any ranked slot beyond the single explicitly-justified transferable-technique entry (§2.5). The standing scientific map is **unchanged** from 07-08 apart from the one new technique lead.

---

## ⭐ New today — genuinely not previously catalogued (technique lead, not a venous slot)

### Integrated Spectral and Depth Compensation (ISDC) for Optimizing sO₂ and Total-Hemoglobin Estimation in Photoacoustic Tomography
...
- **Relevance (as a venous measurement):** **4 / 10** — it does **not** measure venous blood; the clinical target is ovarian-lesion malignancy (82 lesions, 67 benign / 15 malignant). **Relevance as transferable enabling technique: ~7/10.**
- **Novelty:** Proposes an **integrated spectral + depth compensation (ISDC)** correction that jointly removes **wavelength-dependent spectral distortion** and **depth-dependent optical attenuation** from PAT signals before spectral unmixing, validated on optical phantoms of known properties and then applied to clinical PAT. Directly attacks the wavelength- and depth-dependent **fluence** problem that biases absolute sO₂/THb at depth.
- **Integration insights:** The single most transferable new result this cycle. Absolute non-invasive **deep-vein** PA-SvO₂ (Topic 2 §2.1, Gao/Zhu IJV) fails for exactly the reason ISDC targets: unknown, wavelength- and depth-varying fluence colours the measured spectrum. ISDC is a fluence-correction sibling to the arterial-prior APM+ method (§2.5) — where APM+ uses a *known-saturation vessel* as an in-situ calibrator, ISDC uses a *modelled spectral+depth correction*. Both are candidate front-ends for a clinically accurate IJV/femoral venous PA oximeter. **Verification:** title/authors/affiliation/journal/volume/DOI (10.1117/1.JBO.31.2.026002) and cohort (82 lesions) cross-confirmed via PMC index + Semantic Scholar/SPIE listings; PMC full text returned 403 to direct fetch today (flagged).
...
- **Integration insights:** Establishes the **absolute-accuracy and outcome-relevance benchmark** any optical SvO₂ method must ultimately match — and does so in exactly the population (heart failure) where a cheap continuous optical surrogate would be most valuable. The internal-reference (RV vs LV) logic recurs across the optical methods below. **Verification:** cross-confirmed via JACC/ScienceDirect listing + PMC record + prior-scan cross-reference; full text 403 (flagged).
...
- **Integration insights:** The modeling backbone (subject-specific MC + DNN inverse) is the template any deep-vein optical SvO₂ method — PA or NIRS — will need for clinical accuracy; the arterial-prior and ISDC fluence-calibration ideas (§2.5, §New-today) slot naturally into it. **Verification:** confirmed via Optica listing + PubMed index.
...
- **Integration insights:** Sets the empirical bias/limits-of-agreement bar optical/PA methods must beat for clinical adoption. **Verification:** confirmed via PubMed + ScienceDirect index.
...
- **Integration insights:** Anchors the practical low-cost peripheral-venous surrogate branch against which optical vessel-resolved methods can be compared. **Verification:** confirmed via PubMed/PMC + Frontiers index.
...
- **Integration insights:** The deep-vein anchor of the whole program; every fluence-correction (§2.5, §New-today ISDC), phantom-metrology (§2.2) and inverse-modeling (§1.3) advance is ultimately in service of making this class of measurement clinically robust. **Verification:** confirmed via arXiv listing + ADS; abstract page 403 to direct fetch (flagged).
...
- **Integration insights:** The high-accuracy optical benchmark for per-vessel venous saturation; its adaptive-spectral-correction philosophy parallels the fluence-correction (§2.5, §New-today ISDC) and phantom-calibration (§2.2) strategies needed for systemic deep-vein optical SvO₂. **Verification:** confirmed via Nature + PMC index.
...
- **Integration insights:** The most concrete recent recipe for absolute PA sO₂ at depth; its "reference-vessel-calibrates-a-neighbour" logic is exactly what a carotid-calibrated IJV venous measurement would use, and it echoes both the RV/LV internal-reference idea in §1.2 and the modelled spectral+depth correction of today's new ISDC paper (§New-today). **Verification:** confirmed via Wiley listing + prior-scan cross-reference; Wiley full text 403 (flagged).

> **Topic 2 sparseness note:** **No new Topic-2 venous-measurement primary this cycle.** The one genuinely-new 2026 paper found today (ISDC, §New-today) is a PA sO₂ *technique* demonstrated on ovarian tissue, catalogued as a transferable technique lead rather than a ranked venous slot. §2.1–2.4 are re-listed landmark/near-term corpus (each flagged previously surfaced); §2.5 is the single transferable-technique entry carried forward.

---

## Standing context leads (not ranked)

- **Wang et al., ISDC PA fluence-compensation (*J. Biomed. Opt.* 31(2):026002, Feb 2026; DOI 10.1117/1.JBO.31.2.026002)** — **new this cycle**; see §New-today. Depth+spectral fluence correction for quantitative PA sO₂/THb; ovarian-lesion demonstration, transferable to deep-vein PA oximetry.
- **arXiv 2512.15394 — "Deep Learning-Driven Quantitative Spectroscopic Photoacoustic Imaging for Segmentation and Oxygen Saturation Estimation" (Hybrid-Net, Dec 2025)** — joint vessel segmentation + sO₂ estimation trained on Monte-Carlo breast-tissue PA data at 700/850 nm. **Already catalogued**; general PA-oximetry method, not venous-specific — carried as a standing technique lead.
...
- **Non-invasive PA imaging of cerebral oxygenation in awake mice (bioRxiv 2025.10.02.679935, Oct 2025)** — label-free through-skull cerebral oxy/deoxy/total-Hb PA in awake rodents; preclinical, not vessel-resolved venous, carried as a standing preclinical lead.
...
Today adds **one new item to the corpus** — the ISDC spectral+depth fluence-compensation method for quantitative PA sO₂/THb (Wang et al., *J. Biomed. Opt.*, Feb 2026) — but **no new paper that actually measures venous blood**. This is the expected baseline for the niche: genuinely novel non-invasive *venous* optical work publishes at very low volume, and most one- to two-day cycles add zero venous primaries. The scientific map of ranked front-line work is therefore **unchanged from 07-08**; the honest read is that the corpus grew by one *enabling-technique* paper, not by a venous result.

That new paper is nonetheless well-placed on the series' central through-line, the **internal-reference / fluence-correction principle**: absolute non-invasive venous saturation is unreachable through a naïve absorption read at depth, and must instead be recovered by correcting the wavelength- and depth-dependent fluence that colours the measured spectrum. Three distinct strategies for that correction now sit side by side — a *known-saturation neighbour vessel* as in-situ calibrator (APM+, §2.5), a *modelled spectral+depth compensation* (ISDC, §New-today), and *per-vessel adaptive spectral references* in retinal oximetry (vis-OCT §2.4, hyperspectral §2.3) — complemented by the compartment-ratio logic of the MRI T₂ reference (§1.2, RV vs LV). The **validation stack** is likewise unchanged: MRI-T₂ iSvO₂ as the outcome-anchored absolute benchmark (§1.2), blood-matched sulfate PA phantoms as bench-side ground truth across the venous 40–75 % range (§2.2), and the retina as the per-vessel optical proving ground (§2.3, §2.4).

**Honest volume note:** zero new venous-specific primaries today; one new transferable PA-oximetry technique. The core gap the series tracks — **non-invasive human deep-vein optical SvO₂ validated at clinical scale** — remains open and is not directly advanced by anything found today, though ISDC and APM+ together sharpen the fluence-correction toolkit any future deep-vein PA-SvO₂ system will need.
